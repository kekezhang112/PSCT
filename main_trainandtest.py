...
def datasetgenerator(conf,log_dir,EXP_ID ='0'):
    ...
    index = Info['index_all'][:, int(EXP_ID) % 10]
    ...
    for i in range(len(ref_ids)):
        train_index.append(i) if (ref_ids[i] in trainindex[:int(math.ceil(0.8 * len(trainindex)))]) else \
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type=int, default=8, # just for example, please change the batchsize as needed
                        help='batch size for training')
    parser.add_argument('--maxepochs', type=int, default=800,
                        help='number of epochs')
    parser.add_argument('--lr', type=float, default=1e-4,
                        help='learning rate')
    parser.add_argument('--config', default='config_trainandtest.yaml', type=str,
                        help='config file path')
    parser.add_argument('--exp_id', default='0', type=str,
                        help='exp id')
    parser.add_argument('--database', default='exampletrainandtest', type=str,
                        help='database name')
    ...
    model.summary()
    train_index,val_index,test_index = datasetgenerator(config,log_dir,args.exp_id)

    if args.phase == 'train':
        nb_imgs_train = len(train_index)
        nb_imgs_val = len(val_index)
        print("Training PSCT Model")
        checkpointdir = os.path.join(os.getcwd(),'checkpoint_example')
        ensure_dir(checkpointdir)
        reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=5, min_lr=1e-8,verbose=1) # change as needed

        train_generator = DataGenerator(train_index,config, b_s, shuffle=True)
        val_generator = DataGenerator(val_index,config, 1, shuffle=False)
        file_path = "{epoch:02d}-{val_loss:.4f}-{loss:.4f}-{val_predictions_loss:.4f}-{val_saliency_loss:.4f}.h5"
        file_path2 = "{epoch:02d}-{val_loss:.4f}-{loss:.4f}.h5"
        model.fit_generator(generator=train_generator,epochs=args.maxepochs,steps_per_epoch= int(1* nb_imgs_train // b_s),
                                validation_data=val_generator, validation_steps= int(1*nb_imgs_val  // 1),
                                callbacks=[EarlyStopping(patience=30), # change as needed
                                           ModelCheckpoint(os.path.join(checkpointdir,file_path2),save_best_only=True),reduce_lr])
